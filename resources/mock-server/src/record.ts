...
async function algosdkAlgodRequests1() {
  // TestNet configuration (using AlgoNode public API)
  const algod = new Algodv2(
    "a".repeat(64),
    // "http://localhost",
  ...
  // GET /v2/devmode/blocks/offset
  // SKIP: DevMode only
  // await algod.getBlockOffsetTimestamp().do();
...
async function algosdkAlgodRequestsWithMainnet() {
  // TestNet configuration (using AlgoNode public API)
  const algod = new Algodv2(
    "a".repeat(64),
  ...
  // // GET /v2/stateproofs/{round}
  // // TODO: find a valid value. Will likely have to be done with localnet
  const stateProofRound = 55675868;
  await algod.getStateProof(stateProofRound).do();
}

async function algosdkAlgodRequestsWithLocalnet() {
